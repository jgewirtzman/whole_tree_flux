...
create_tree_visualization <- function() {
  ...
  create_tree_profile <- function() {
    ...
    # First, calculate area percentages for all heights to determine color range
    ...
    # Calculate area percentage for each segment midpoint
    ...
    # Create color palette based on area percentages (red = low %, blue = high %)
    # Normalize area percentages to 0-1 range for color mapping
    max_area <- max(segment_areas)
    min_area <- min(segment_areas)
    normalized_areas <- (segment_areas - min_area) / (max_area - min_area)
    
    # Create colors: red for low area %, blue for high area %
    ...
    p1 <- ggplot() +
      ...
      scale_fill_identity() +
      coord_fixed(ratio = 0.5) +  # Adjust for 5x stretch
  ...
  create_capture_curve <- function(area_range) {
    ...
    p2 <- ggplot() +
      ...
      geom_line(data = capture_data, 
                ...
                linewidth = 3, alpha = 0.8, color = "black", linetype = "dashed") +
      
      # Line annotations
      annotate("text", x = 82, y = 22, label = "% of Stem", 
               fontface = "bold", size = 4, color = "black") +
      annotate("text", x = 35, y = 22, label = "% of Tree", 
               fontface = "bold", size = 4, color = "black") +
      ...
      geom_text(data = sampling_points, 
                aes(x = pmax(stem_percent + 8, 8), y = height, 
                    label = paste0(label, "\n(", round(stem_percent, 1), "%)")), 
                hjust = 0, color = "black", fontface = "bold", size = 3.5) +
      ...
      geom_vline(xintercept = 100, 
                 color = "#4575b4", linewidth = 1, linetype = "dotted", alpha = 0.7) +
...
cat("Creating tree visualization with line annotations instead of legend...\n")
