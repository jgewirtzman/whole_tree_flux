...
create_tree_visualization <- function() {
  ...
  create_tree_profile <- function() {
    ...
    p1 <- ggplot() +
      ...
      scale_y_continuous(breaks = seq(0, 26, 5),
                         limits = c(-2, 27),  # Extend below 0 to show full ellipses
  ...
  create_capture_curve <- function(area_range) {
    ...
    p2 <- ggplot() +
      ...
      # Line annotations
      annotate("text", x = 76, y = 21, label = "% of Stem", 
               fontface = "bold", size = 4, color = "black") +
      annotate("text", x = 40, y = 24, label = "% of Tree", 
               fontface = "bold", size = 4, color = "black") +
      ...
      scale_y_continuous(breaks = seq(0, 26, 5),
                         limits = c(-2, 27),  # Match left panel limits
                         name = "Height above ground (m)") +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
            legend.position = "none",
